...
2. **Bases de mots-clés multi-domaines, deux sources combinées dès la v1** (Matthieu : « il y a des offres en anglais pour des sites hors de France, fais les deux ») :
   ...
   - **ESCO** (Union Européenne) — taxonomie multilingue (FR/EN inclus), tous secteurs, dataset téléchargeable intégralement en CSV (pas d'appel API à chaque analyse, stocké en local une fois pour toutes).
